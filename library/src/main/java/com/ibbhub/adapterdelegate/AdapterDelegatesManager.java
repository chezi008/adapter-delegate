 ...
 * #setFallbackDelegate(IDelegate)} that will be used if no {@link AdapterDelegate} is
 ...
 */
public class AdapterDelegatesManager<T> implements IDelegateManager<T> {
   ...
   */
  protected SparseArrayCompat<IDelegate<T>> delegates = new SparseArrayCompat();
  protected AbsFallbackAdapterDelegate<T> fallbackDelegate;
   ...
   * unused</b>
   *
   * Internally calls {@link #addDelegate(int, boolean, IDelegate)} with
   * allowReplacingDelegate = false as parameter.
   *
   * @param delegate the delegate to add
   * @return self
   * @throws NullPointerException if passed delegate is null
   * @see #addDelegate(int, IDelegate)
   * @see #addDelegate(int, boolean, IDelegate)
   */
  @Override
  public AdapterDelegatesManager<T> addDelegate(@NonNull IDelegate<T> delegate) {
   ...
   * use {@link #addDelegate(IDelegate)} where a viewtype will be assigned manually.
   * @param delegate the delegate to add
   * @return self
   * @throws NullPointerException if passed delegate is null
   * @see #addDelegate(IDelegate)
   * @see #addDelegate(int, boolean, IDelegate)
   */
  @Override
   ...
   * @see #addDelegate(IDelegate)
   * @see #addDelegate(int, IDelegate)
   * @see #setFallbackDelegate(IDelegate)
   */
  @Override
   ...
   */
  @Override
  public AdapterDelegatesManager<T> removeDelegate(@NonNull IDelegate<T> delegate) {
  ...
  public int getItemViewType(@NonNull T items, int position) {
    ...
    for (int i = 0; i < delegatesCount; i++) {
      IDelegate<T> delegate = delegates.valueAt(i);
  ...
  @NonNull public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    IDelegate<T> delegate = getDelegateForViewType(viewType);
    if (delegate == null) {
  ...
  public void onBindViewHolder(@NonNull T items, int position,
    ...
    IDelegate<T> delegate = getDelegateForViewType(holder.getItemViewType());
    if (delegate == null) {
   ...
   */
  @Override
  public int getViewType(@NonNull IDelegate<T> delegate) {
    if (delegate == null) {
   ...
   */
  @Override
  @Nullable public IDelegate<T> getDelegateForViewType(int viewType) {
   ...
   */
  @Override
  @Nullable public AbsFallbackAdapterDelegate<T> getFallbackDelegate() {
